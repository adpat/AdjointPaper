...
for every $\varphi \in \mathbf{C}^1_c(\R^+\times \R; \R).$
	\item $\cvar$ satisfies the Kru\v{z}hkov entropy condition\footnote{add reference to Kruzhkov paper} on $(\R^+\times\R)$, i.e.,for every $k\in \R$ and for all $\varphi \in \mathcal{C}^1_c(\R^+\times \R;\R^+),$
\begin{eqnarray}
\label{eq:kruzhkov}
...
A network
% of hyperbolic conservation laws such as~(\ref{eq:cons})
